---
title: 程序设计语言导览
description: 面向新生的 15 种常见程序设计语言对比、特色代码与方向选择建议
category: start
audience:
  - 本科新生
...
本页选取 15 种在课程学习、Web、后端、系统软件和移动端中常见的程序设计语言，帮助新生认识不同语言的设计取向和典型用途。它不是严格的热度排行榜，也不表示每种语言都需要学习。
...
| 语言         | 类型与运行方式                  | 代表方向                        | 新生建议                     |
| ------------ | ------------------------------- | ------------------------------- | ---------------------------- |
| C            | 静态类型、原生编译              | 程序设计基础、嵌入式、底层软件  | 优先跟随课程掌握             |
| C++          | 静态类型、原生编译              | 算法、游戏引擎、高性能软件      | 掌握 C 基础后学习            |
| Python       | 动态类型、解释执行              | AI、数据分析、自动化、Web 后端  | 适合快速完成实践项目         |
| Java         | 静态类型、JVM                   | 企业后端、Android 基础生态      | 适合学习面向对象与大型工程   |
| JavaScript   | 动态类型、浏览器或 Node.js      | Web 前端、全栈与开发工具        | Web 方向的基础语言           |
| TypeScript   | 静态类型增强、编译为 JavaScript | 大型前端、全栈、SDK             | 建议在 JavaScript 之后学习   |
| PHP          | 动态类型、解释执行              | Web 后端、CMS、电商             | 适合维护和开发传统 Web 项目  |
| Ruby         | 动态类型、解释执行              | Rails、脚本、快速原型           | 适合观察高表达力语言设计     |
| C#           | 静态类型、.NET                  | 企业应用、云服务、Unity         | 适合 .NET 与游戏开发方向     |
| Shell / Bash | 动态脚本、命令解释              | Linux、CI/CD、运维自动化        | 所有开发方向都值得掌握基础   |
| Go           | 静态类型、原生编译              | 云原生、网络服务、CLI           | 适合后端和基础设施方向       |
| Rust         | 静态类型、原生编译              | 系统软件、WebAssembly、基础设施 | 建议具备内存与系统基础后学习 |
| Kotlin       | 静态类型、JVM 或多平台          | Android、JVM 后端、多平台       | 移动端和 JVM 方向可选        |
| Swift        | 静态类型、原生编译              | Apple 平台应用                  | iOS、macOS 方向可选          |
| Dart         | 静态类型、JIT/AOT               | Flutter 多端应用                | Flutter 方向可选             |

## 新生怎么选

- 课程学习：先看 [C 语言程序设计](/study/c) 和 [C/C++ 入门](/study/cpp-basics)，重点理解变量、控制流程、函数、数组、指针和编译过程。
- Web 开发：从 [Web 前端设计](/study/web-frontend) 进入 HTML、CSS 和 JavaScript，再学习 TypeScript。
- 数据与人工智能：用 Python 完成数据处理、自动化脚本和简单模型调用，不必一开始追求复杂框架。
- 后端与工程：Java、C#、Go 都适合构建服务端项目，选择一门深入比同时浅尝多门更有效。
- 系统与性能：先理解 C/C++ 的内存与编译模型，再接触 Rust 等强调安全和性能的语言。
- 移动端：Android 可关注 Kotlin，Apple 平台使用 Swift，Flutter 多端开发使用 Dart。

GitHub 的语言活跃度会随统计口径和时间变化。根据 GitHub Octoverse 2025，TypeScript 在 2025 年 8 月首次成为 GitHub 月度贡献者数量最多的语言；本页只把该报告作为观察生态趋势的参考，不据此给出“最好语言”排名。
